...
### Effects: Everything Is Transformation

The most fundamental concept in the system is the **effect** — a claim that something changed in the world, at a specific place and time. Everything else is built on effects.
...
- "Starting March 1st, a workshop space will be available weekdays 9-5"

Every effect has what changed, where and when, who says so, and a lifecycle (more on this below). There's no fundamental difference between a human contribution and a natural event — both are transformations witnessed and asserted.

### Capacities: The Present State of Things

A **capacity** is a snapshot — the current state of an entity, derived from all the effects that have acted on it. "There are 20 drills at the downtown library" is the net result of every acquisition, donation, loan, breakage, and return. The number 20 is what you get when you fold all those effects together.
...
Capacities exist in all temporal modes. A present capacity ("I have 20 drills now") is a snapshot of accepted effects. A future capacity can be either a **commitment** ("Starting March 1st, I'll have a space available" — someone is accountable) or a **projection** ("At current rates, we'll have 12 drills by April" — the system extrapolated, nobody promised). The distinction matters: you can hold someone to a commitment, not to an extrapolation.

### Needs: What's Wanted

A **need** is the same shape as a capacity — a type, quantity, location, schedule — but expressing what's wanted rather than what's available. Needs and capacities are matched across the same dimensions.
...
## The Lifecycle of Effects

Plans are not reality. Someone says they'll contribute 10 hours. Did they? Someone promises a venue. Did it actually open on time? The effects system tracks the gap between projection and actuality.
...
```
Effects assert transformations in space-time
    (what changed, where, when, witnessed by whom)
                    ↓
Capacities snapshot current state from effects
    (checkpoints so you don't replay everything)
                    ↓
Needs express what's wanted (same shape as capacities)
                    ↓
Matching finds feasible, desired connections
    (across time, space, quantity, skills, travel, affinity, continuity)
                    ↓
Commons organize contributions into coordinated activities
    (slots define what's needed, effects fill them)
                    ↓
Lifecycle tracks the gap between plan and reality
    (projected → judged → accepted or not)
                    ↓
Derivation computes state from accepted effects
    (not stored — always freshly computed)
                    ↓
Metabolism shows sustainability across space and time
    (production vs consumption, as a field not a scalar)
                    ↓
State predicates propagate changes
    (if state breaks a condition, every effect watching it knows)
```
...
- Where and when intervention is needed before things break down
...
- Between Reality and Transformation
- Conditions/constraints on effects which can be applied
    - without limiting expression of reality
        - although we might limit whose perspective we count
    - we must limit whose effects are counted as allocation???
        - i.e. the commons itself must choose who to respect? with respect the entity effects?
        - or does the entity define who governs it???
...
## Process
- Space/Time as just one example of orgnazing process
- example: turns/rounds etc. can organize "flow" of process.
- time-space window vs before/after/during/when
    - control flow primitives, sync async, concurrent, parallel
- make effects: 
    - current approach:effect will happen at a certain time, in a certain space, with a certain quantity, by a certain entity, witnessed by a certain entity
    - proposed extension: effect/event will/will-not happen before/after/during/when other effect/event happens

## Commons and Effects
- Effects initiates in a commons?
- Effects composed in a commons? composition of effects = organization of labor + nature
- Emergent effects from the composition by default sum of parts, but with modify upon pending.

- DEFAULT: Commons = aggregation of effects
- REAL: Emergent effects from the composition
- VERY USEFUL: keep track of the REAL effects of certain compositions, so we can suggest them when the same pattern arrives!

- GOVERNANCE: of effect is same as governance of commons? i.e. each commons has its own processing queue? With delegated acceptance?
    - risk of production failure, is directly related to what effects are accepted?
    - Could effect information propogate via the commons graph then????????????
    - So makes sense to localize processing at that level? Negative externalities? coca cola? down-stream disagrees? or would add to the consequences, you are ignoring X effect?! Others merge views they consider relevant?
        - Whose views of effects am i considering (i.e. i subscribe), what is my merge strategy?
            - EXTENSION: helps the merge problem, when an effect references another, saying i EXTEND this with these (i.e. you missed something)
                - difference between saying:
                    - alice: + 2 tree
                    - bob: +3 tree 
                    - ??? overlap? addition? whats the merge strategy?
                        - if bob says: alice + 1 tree
                        - then we know that bob is aware of alice's effect and is adding to it
                            - no conflict!
                            - MODIFICATION of alice's effect by bob
                    - EFFECT EXTENSION/COMPOSITION as distinct from COMMONS COMPOSITION???????????????????
    - EXPRESSION of effects
        - 
    - CAUSE of EFFECT different from expression of effect?
    - Commons is Cause?
    - Overdetermination: Overdetermination occurs when a single observed effect is determined by multiple causes, any one of which alone would be conceivably sufficient to account for ("determine") the effect.

    - People participate or not based on whether they agree with the commons governance view of effects? Is my commons in delusion!
        - governance via:
            - preset, template etc.

## Dependencies:
- Are effect dependencies and Commons composition the same thing?

## Composition
- what pieces give us simply composibility of ?
